...
- Statistical methods for obtaining **reliable information** from samples of populations with <mark>unknown SD $\sigma$</mark> (*không biến độc phân tán dữ liệu ra sao*!!!).
...
Assume that $x$ has normal distribution with mean $\mu$. For sample size $n$, with the sample mean $\bar{x}$ and sample standard deviation $s$, the **t variable**,
...
- The **shape** of the t distribution <mark>depends only on $n$</mark>, **provided** the basic variable $x$ has a **normal distribution**. 
