...
<div class="p-mark">
$$
PMF: p_X(k,n,p)=\Pr(k;n,p)=\Pr(X=k)={\binom {n}{k}}p^{k}(1-p)^{n-k}
$$
</div>

<div class="p-mark">
$$
CDF: F(k;n,p)=\Pr(X\leq k)=\sum _{i=0}^{\lfloor k\rfloor }{n \choose i}p^{i}(1-p)^{n-i}
$$
</div>
...
- **Variance**: **$\Leftarrow$ Chưa giải thích!**{:.tred}
