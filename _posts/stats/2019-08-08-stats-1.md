...
$\Rightarrow$ $X+X$ chính là tổng điểm trên mặt của 2 lần tung. Còn $X\times X$ chính là tích điểm trên mặt của 2 lần tung. Ví dụ, $X=1$ (1 lần tung được 1 nút), còn $Z=2X=5$ (2 lần tung được 5 nút) và $Z=X^2=4$ (2 lần tung được 4 nút). **Tuy nhiên probability distribution sẽ thay đổi** với $2X$ và $X^2$.
...
- **Convolution** (tính chập): suppose $Z=X+Y$ and $X=x$ then the event $Z=z$ is equivalent to **union** of $(X=x)$ **and** $(Y=z-x)$. $\Rightarrow$ Có công thức tính theo $f\_X$. 
...
- *Dấu bằng thứ 2*: Liệu sau khi $X^2$ thì trung bình của nó có giống với trước khi nó bình phương hay không? **$\Leftarrow$ Vẫn chưa thuyết phục!**{:.tred}
...
![Uniform distribution]({{img-url}}/uniform-distribution.svg){:.w-400}
...
![Normal Distribution: PDF]({{img-url}}/nd-1.png){:.w-500}
*Probability density function*
</div>
<div class="col s12 l6" markdown="1">
![Normal Distribution: CDF]({{img-url}}/nd-2.png){:.w-500}
...
- Trò **Quincunx** cho 1 ví dụ của cái normal dist này (các viên bi rơi ngẫu nhiên xuống 1 tam giác có nhiều chướng ngại)
- **[68-95-99.7 rule](https://en.wikipedia.org/wiki/68%E2%80%9395%E2%80%9399.7_rule)** (**Empirical rule**)
...
**[??]**{:.tred} *Dấu hiệu nhận biết của normal distribution là gì?* **$\Leftarrow$ Vẫn chưa thuyết phục!**{:.tred}
...
- When a RV has only 2 independent outcomes (giống như tung đồng xu). Each trial has the same probability.
- **Usage**: The binomial distribution is often used in social science statistics as a building block for models for dichotomous outcome variables, like whether a Republican or Democrat will win an upcoming election, whether an individual will die within a specified period of time, etc ([ref](https://www.investopedia.com/terms/b/binomialdistribution.asp)).
...
- **Expected value** (mean): $E = n\_{trials} \times p\_{success}$
- **Variance**: $np(1-p)$ where $p$ is the proba of the success

**[??]**{:.tred} *Why we have E and EX like above?* **$\Leftarrow$ Vẫn chưa thuyết phục!**{:.tred}
